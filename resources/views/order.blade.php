...
<body>
  <div class="bodyBox">
    <div class="main">
      <form class = "container" method="POST" action="{{ action([App\Http\Controllers\OrderController::class, 'storeOrder']) }}">
        @csrf
        <div class="row py-4 px-5">
          <div id="type" class="col-lg-12 col-md-12 col-sm-12 form-check">
            <label> Type
              <br>
              <label><input name="entree_type" type="radio" value="Burger or Sandwich" required> Burger or Sandwich </label>
              <br>
              <label><input name="entree_type" type="radio" value="Wrap" required> Wrap </label>
            </label>
          </div>
        </div>
        <div class="orderForm" style="visibility: hidden">
          <div class="row p-5 pb-5 px-5">
            ...
            <div id="entree" class="col-lg-6 col-md-6 col-sm-12" required>
              ...
              <br>
              <select class = "form-select own" name ="entree" id="dropMenu" required>
                <option value="" selected style="display: none" label="Select an Option" ></option>
                @foreach ($entrees as $key => $entree)
                    <option class="{{$entree->entree_type}}" value="{{ $entree->id}}"> {{ ucwords($entree->entree_name)}} </option>
                @endforeach
              </select>
            </div>
          </div>
            ...
            <div class="col-lg-4 col-md-4 col-sm-12">
                <div class="form-check">
                    ...
                    @foreach ($condiments as $key=>$condiment)
                        <label> <input type="checkbox" value="{{$condiment->id}}" name="condiments[]"> {{$condiment->condiment_name}} </label>
                        <br>
                ...
                <div class="form-check">
                  <label> Toppings
                    @foreach ($toppings as $key=>$topping)
                        <br>
                        <label><input type="checkbox" name="toppings[]" value="{{$topping->id}}"> {{$topping->topping_name}} </label>
                    @endforeach
                  </label>
                </div>
            </div>
                ...
                <div id="cheese" class="form-check">
                  <label> Cheese
                    @foreach ($cheeses as $key=>$cheese)
                        <br>
                        <label> <input type="radio" name="cheese" value="{{$cheese->id}}" required> {{$cheese->cheese_name}} </label>
                    @endforeach
                  </label>
                </div>
              </div>
              ...
              <div class="row">
                <div id="fries" class="form-check">
                  <label class = "fries"> Fries
                    <br>
                    <label> <input type="radio" name="fries" value ="yes" required> Yes </label>
                    <br>
                    <label> <input type="radio" name="fries" value ="no" required> No </label>
                  </label>
                </div>
              </div>
            </div>
